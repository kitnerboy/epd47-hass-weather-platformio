...
bool DecodeWeather(WiFiClient& json) {
  ...
  Serial.println(" Decoding weather data");
  {
    ...
    WxConditions[0].Main0       = root["state"].as<const char*>();        Serial.println("Main: " + String(WxConditions[0].Main0));
...
//    WxConditions[0].Icon        = root["weather"][0]["icon"].as<char*>();        Serial.println("Icon: " + String(WxConditions[0].Icon));
    WxConditions[0].Temperature = root["attributes"]["temperature"].as<float>();              Serial.println("Temp: " + String(WxConditions[0].Temperature));
    WxConditions[0].Pressure    = root["attributes"]["pressure"].as<float>();          Serial.println("Pres: " + String(WxConditions[0].Pressure));
    WxConditions[0].Humidity    = root["attributes"]["humidity"].as<float>();          Serial.println("Humi: " + String(WxConditions[0].Humidity));
...
//    WxConditions[0].High        = root["main"]["temp_max"].as<float>();          Serial.println("THig: " + String(WxConditions[0].High));
    WxConditions[0].Windspeed   = root["attributes"]["wind_speed"].as<float>();             Serial.println("WSpd: " + String(WxConditions[0].Windspeed));
    WxConditions[0].Winddir     = root["attributes"]["wind_bearing"].as<float>();               Serial.println("WDir: " + String(WxConditions[0].Winddir));
    WxConditions[0].Cloudcover  = root["cloud_coverage"]["cloud_coverage"].as<int>();               Serial.println("CCov: " + String(WxConditions[0].Cloudcover)); // in % of cloud cover
...
bool DecodeForecast(WiFiClient& json) {
  Serial.print(F("\nCreating object...and "));
  DynamicJsonDocument doc(64 * 1024);                      // allocate the JsonDocument
  DeserializationError error = deserializeJson(doc, json); // Deserialize the JSON document
  if (error) {                                             // Test if parsing succeeds.
    Serial.print(F("Forecast deserializeJson() failed: "));
    Serial.println(error.c_str());
    return false;
  }
  // convert it to a JsonObject
  JsonObject root = doc.as<JsonObject>();
  Serial.println(" Decoding forecast data");
  {
    //Serial.println(json);
    Serial.print(F("\nReceiving Forecast period - ")); //------------------------------------------------
    JsonArray list                  = root["attributes"]["forecast"];
    for (byte r = 0; r < max_readings; r++) {
      ...
      WxForecast[r].Temperature       = list[r]["temperature"].as<float>();              Serial.println("Temp: " + String(WxForecast[r].Temperature));
...
bool obtainWeatherData(WiFiClient & client, const String & RequestType) {
  const String apiHost = "http://" + hass_ip +  ":" + hass_port;
  const String apiPath_weather = "/api/states/weather.forecast_home";
  const String apiPath_forecast = "/api/states/sensor.hourly_forecast_home_sensor";

  const String fullUrl_weather = apiHost + apiPath_weather;
  const String fullUrl_forecast = apiHost + apiPath_forecast;
  

  client.stop(); // close connection before sending a new request

  HTTPClient http;
  // 1. Инициализация и начало сессии
  http.begin(fullUrl_weather); 

  // 2. Установка заголовков (Headers)
  // Заголовок Content-Type (хотя для GET не всегда обязателен, соответствует вашему curl-запросу)
  http.addHeader("Content-Type", "application/json"); 

  // Заголовок Authorization: Bearer <TOKEN>
  String authHeader = "Bearer " + String(hass_token);
  http.addHeader("Authorization", authHeader);
  
  Serial.println("Отправка GET-запроса погоды...");
  Serial.println(fullUrl_weather);
  ...
  if (httpResponseCode == HTTP_CODE_OK) {
    if (!DecodeWeather(http.getStream())) {
      Serial.println("Weather decode failed");
      return false;
    }
  } else {
    // Ошибка HTTP-запроса (например, -1 для таймаута)
  ...
  // 1. Инициализация и начало сессии
  http.begin(fullUrl_forecast); 
  http.addHeader("Content-Type", "application/json"); 
  http.addHeader("Authorization", authHeader);
  
  Serial.println("Отправка GET-запроса прогноза...");
  Serial.println(fullUrl_forecast);
  ...
  // 4. Обработка ответа
  if (httpResponseCode == HTTP_CODE_OK) {
    if (!DecodeForecast(http.getStream())) {
      Serial.println("Forecast decode failed");
      return false;
    }
  } else {
    // Ошибка HTTP-запроса (например, -1 для таймаута)
...
void drawString(int32_t x, int y, String text, alignment align) {
